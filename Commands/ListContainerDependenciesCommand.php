...
use Bertshang\Porto\Transformers\ComposerTransformer;
 ...
 * Parses all files in the Container. This is needed due to the implemented porto calls.
...
class ListContainerDependenciesCommand extends ConsoleCommand
{
    ...
    protected $signature = 'porto:list:dependencies {containerPath}';
    ...
    private function getDependencies($path, $filterOwnContainer = false)
    {
        ...
        foreach ($recursiveIteratorIterator as $file) {
            if (!$file->isDir()) {
                $portoCalls = $this->getContainerFromApiatoCall($file->getPathName());
                $imports = $this->getContainerFromUseStatement($file->getPathName());

                if (isset($portoCalls['containers'])) {
                    if ($filterOwnContainer) {
                        $portoCalls['containers'] = array_diff($portoCalls['containers'], [$ownContainerName]);
                    }

                    foreach ($portoCalls['containers'] as $container) {
                        $filesInContainers['portoCalls'][$container][] = $file->getPathName();
                    }
                }

...
namespace Bertshang\Porto\Traits;

use Bertshang\Porto\Abstracts\Requests\Request;
use Bertshang\Porto\Abstracts\Transporters\Transporter;
...
trait CallableTrait
{
    ...
    {
        // in case passing porto style names such as containerName@classType
        if ($this->needsParsing($class)) {
            ...
            if (Config::get('porto.logging.log-wrong-porto-caller-style', true)) {
                Log::debug('It is recommended to use the porto caller style (containerName@className) for ' . $class);
            }
        }
    ...
    /**
     * If it's porto Style caller like this: containerName@someClass
     *
     * @param        $class

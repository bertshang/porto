...
namespace Bertshang\Porto\Loaders;
...
trait RoutesLoaderTrait
{
    ...
    {
        return Config::get('porto.api.prefix') . (Config::get('porto.api.enable_version_prefix') ? $this->getRouteFileVersionFromFileName($file) : '');
    }
    ...
    private function getRateLimitMiddleware()
    {
        ...
        if (Config::get('porto.api.throttle.enabled')) {
            $rateLimitMiddleware = 'throttle:' . Config::get('porto.api.throttle.attempts') . ',' . Config::get('porto.api.throttle.expires');
        }
